<template>
<section id="main-program">
    <p class="title">Who We Are?</p>
    <p class="title">Passionate builders</p>
    <p class="title"> We Work tiressly and stay true to our core values</p>
    <hr />
    <div class="main-program">
        <div class="lecture">
            <h3>Adaptive Process</h3>
            <p>
                We use a proven Agile methodology to always deliver result beyond expectaions.
            </p>

        </div>

        <div class="ccexhibition">
            <h3>Collaborative & Transparent</h3>
            <p>
                We have a passion for finding solutions together with our clients; not just for them.
            </p>
        </div>
        <div class="forum">
            <h3>Quality Obsession</h3>
            <p>
                We get it right the first through focus and attention to details.
            </p>
        </div>
        <div class="workshop">
            <h3>Ownership Mentality</h3>
            <p>
                Every project is treated with trust, a sense of ownership, and drive.
            </p>
        </div>
    </div>
</section>
</template>

<script>
export default {
    name: 'MyFooter'

}
</script>

<style>
@import '../assets/style.css';
</style>
